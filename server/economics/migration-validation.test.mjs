...
assert.equal((await db.query(`SELECT has_function_privilege('public','public.import_x_saved_item_unmetered(uuid,text,jsonb,uuid)','EXECUTE') AS allowed`)).rows[0].allowed,false);
assert.equal((await db.query(`SELECT has_function_privilege('anon','public.import_x_saved_item_unmetered(uuid,text,jsonb,uuid)','EXECUTE') AS allowed`)).rows[0].allowed,false);
assert.equal((await db.query(`SELECT has_function_privilege('authenticated','public.import_x_saved_item_unmetered(uuid,text,jsonb,uuid)','EXECUTE') AS allowed`)).rows[0].allowed,false);
assert.equal((await db.query(`SELECT has_function_privilege('service_role','public.import_x_saved_item_unmetered(uuid,text,jsonb,uuid)','EXECUTE') AS allowed`)).rows[0].allowed,true);
await db.exec(`SET request.jwt.claims = '{"role":"authenticated"}'; SET ROLE authenticated`);
await assert.rejects(db.query(`SELECT * FROM import_x_saved_item_unmetered($1,$2,$3::jsonb,$4)`,[two,'auth-denied',JSON.stringify(row),jobId]));
await db.exec(`RESET ROLE; SET request.jwt.claims = '{"role":"anon"}'; SET ROLE anon`);
await assert.rejects(db.query(`SELECT * FROM import_x_saved_item_unmetered($1,$2,$3::jsonb,$4)`,[two,'anon-denied',JSON.stringify(row),jobId]));
await db.exec(`RESET ROLE; SET request.jwt.claims = 'not-json'`);
await assert.rejects(db.query(`SELECT * FROM import_x_saved_item_unmetered($1,$2,$3::jsonb,$4)`,[two,'invalid-claims-denied',JSON.stringify(row),jobId]));
